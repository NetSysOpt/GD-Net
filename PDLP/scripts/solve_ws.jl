...
function solve_instance_and_output(
  ...
  function inner_solve()
    ...
    primal_size = length(lp.variable_lower_bound)
    primal = zeros(Float64, primal_size)
    primal_str = ""
    f = open("/home/lxyang/git/GD-Net/predictions/primal.sol", "r")
    while ! eof(f)  
        s = readline(f)    
        primal_str = split(s," ")
    end
    ...
    running_time = @elapsed begin
      output::FirstOrderLp.SaddlePointOutput =
        FirstOrderLp.optimize_ws(parameters, lp, x_ws, stepsize, primalweight)
    end
